...
3. **Three Mandatory Pillars of SDD Execution**:
   ...
   b) **Сначала Тест через Локальную LLM (TDD Red Phase Offloading)**:
      - TDD tests (`*.spec.ts`, `test_*.py`) MUST be generated by delegating to the Local LLM worker via `ask_local_llm.py` before implementation code is written.
      - The Orchestrator verifies that the generated test fails (Red Phase) before proceeding.

   c) **DEFAULT: 100% Local LLM Delegation & Zero-Code Lock (По умолчанию только локальная LLM)**:
      - BY DEFAULT, ALL code writing, component creation, refactoring, TDD test writing, and file generation MUST be executed STRICTLY by querying the local LLM (Ollama / LM Studio) via `python scripts/ask_local_llm.py`.
      - **Zero-Code-in-Orchestrator-Context Protocol**: The Cloud Orchestrator MUST NEVER directly write or edit source implementation files (`.ts`, `.html`, `.scss`, `.py`, `.go`) using code edit tools. The Cloud Orchestrator writes ONLY `.openspec/*.md` Markdown specifications.
   ...
   d) **Физический вызов и автоматический сборщик промптов (Prompt Assembly & No Text Simulation)**:
      - The Orchestrator MUST use `python scripts/build_worker_prompt.py --persona ... --instruction ... --rules ... --out prompt.md` to assemble worker prompts automatically.
      - The Orchestrator MUST physically invoke `run_command` with `python .../ask_local_llm.py` to query LM Studio / Ollama and read the generated file output. Text simulation in chat is FORBIDDEN.

   e) **Локальный цикл самоисправления (Local Worker Ref Loop & Auto-Retry)**:
      - After a Worker LLM generates code, run tests (`npm test`, `pytest`).
      - If tests fail, feed the error trace back to the local LLM via `ask_local_llm.py` with `debug_worker_persona.md` to fix the code on the local side (up to 3 retries) before escalating to the user.
