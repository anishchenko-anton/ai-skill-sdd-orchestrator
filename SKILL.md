...
- **Строгое разделение ролей (Создатель vs. Аудитор)**: Оркестратор (облачная модель) ОБЯЗАН писать тесты до начала написания бизнес-логики. Рабочему агенту (Worker) делегируется только написание кода реализации. Исполнителю **категорически запрещено** изменять или удалять файлы тестов.
- **Spec-Driven Testing**: Оркестратор генерирует тесты, опираясь *исключительно* на утвержденный файл `specs.md` (на паттерны `when... then...`). Запрещено анализировать написанный код реализации при создании тестов — базой выступают только бизнес-требования.
