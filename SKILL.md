...
- **Strict Prohibition on Hardcoded IP Addresses & Subnets**: FORBIDDEN to hardcode static IP addresses (`192.168.121.2`, `147.135.208.25`), CIDR ranges (`192.168.121.0/24`), or VPN endpoints inside application source code, service files, or string templates (`confContent`).
- **No Fallback IP Literals (STRICT)**: FORBIDDEN to use hardcoded IP strings as fallback defaults in code (e.g. `process.env.WG_CLIENT_IP || '192.168.121.2'`, `process.env.WG_ALLOWED_IPS || '192.168.121.0/24'`). Missing environment variables MUST trigger dynamic calculation or an explicit initialization error — NEVER fall back to hardcoded IP strings.
- **Prohibition of Dummy Key Placeholders (STRICT SECURITY RULE)**: Returning fake placeholder strings (e.g. `return 'VISNET_DEV_SERVER_PUBLIC_KEY_PLACEHOLDER='`, `jwtSecret || 'secret'`) for WireGuard keys, secrets, or certificates is STRICTLY FORBIDDEN. Missing security credentials MUST fail startup immediately (`throw new Error(...)`) or dynamically read generated keypairs at deploy time.
- **Dynamic Configuration Enforcement**: WireGuard config generators, backend services, and network modules MUST inject subnets, endpoints, and interface IPs dynamically from environment variables (`process.env.WG_SUBNET`, `process.env.SERVER_PUBLIC_IP`, `process.env.WG_ENDPOINT`), database settings, or dynamic IP allocation logic.
