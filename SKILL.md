...
- [ ] Нет инлайн `template:` (Angular — только `templateUrl`)
- [ ] Нет хардкод URL (`localhost:3000` и т.д.)
- [ ] Нет заглушек CLI (`return true;`, `@Service()`, пустые файлы)
- [ ] Security Checklist пройден (см. `references/security_checklist.md`)
- [ ] Удалены файлы-скаффолдинги, сгенерированные CLI, но не используемые
