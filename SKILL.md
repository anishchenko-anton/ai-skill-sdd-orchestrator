...
- **Подключение фреймворк-специфичных правил**: При постановке задачи локальной LLM (Worker) Оркестратор обязан добавлять к промпту инструкцию вида "Ознакомься с правилами фреймворка из файла `.agents/rules/<название>-rules.md`", чтобы исполнитель учитывал актуальные бест-практисы и ограничения (например, запрет на инлайновые шаблоны в Angular).
